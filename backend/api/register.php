...
// Validation NNI : 14 chiffres
if (!preg_match('/^\d{10}$/', $nni)) {
    http_response_code(400);
    echo json_encode(["message" => "Le NNI doit contenir exactement 14 chiffres"]);
    exit;
}
...
// Vérifier que le NNI existe dans la table
...
if (!$etudiant) {
    ...
    echo json_encode(["message" => "NNI inexistant"]);
    exit;
}

if (!empty($etudiant['email']) || !empty($etudiant['mot_de_passe'])) {
    http_response_code(409);
    echo json_encode(["message" => "NNI déjà utilisé pour un compte"]);
    exit;
}
...
try {
    $stmt = $pdo->prepare("
        ...
        WHERE nni = ?
    ");
    $stmt->execute([$nom, $prenom, $email, $hashedPassword, $nni]);
